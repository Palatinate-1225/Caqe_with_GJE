...
QBFResult QBFSolver::solve_recursive(const std::vector<Formula>& prefix, int depth, std::vector<std::vector<int>> matrix, std::vector<bool> top, int next_ID) {
    ...
    if (matrix.empty()||is_empty) return Q_SAT;
    ...
    while (true) {
        ...
        // 5. 簡化矩陣 (Substitution)
        auto simplified_matrix = simplify(matrix, currentQ);

        // 5.5 處理下一詞傳遞的資訊
        std::vector<bool> next_top(number_of_clauses, false);
        for (int i = 0; i < number_of_clauses; i++){
            b.insert({(next_ID + i), assignment[i + next_ID]});
            next_top[i] = (assignment[i + next_ID] > 0 && !top[i])? false : true; //
        }

        // 6. 遞迴求解內層
        QBFResult recursiveRes = solve_recursive(prefix, depth + 1, simplified_matrix, next_top, next_ID);
...
std::vector<std::vector<int>> QBFSolver::simplify(const std::vector<std::vector<int>>& matrix, const Formula& currentQ) {
